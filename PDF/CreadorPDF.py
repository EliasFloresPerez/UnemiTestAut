...
class CrearPdf():
    ...
    def CrearHoja(self,datos):
        ...
        self.pdf.image("PDF/LogoUnemi.png", x= (210/2) -25, y=5, w=50, h=20, type="PNG")
        ...
        self.pdf.text(20,85,"Rellene la respuesta correcta en su totalidad, evite seleccionar dos respuestas.")

        #Colocamos el fotter
        self.pdf.text((210/2) - 60,280,"Universidad Estatal de Milagro Todos los Derechos Reservados *")
        ...
        if preguntas > 10:
            ...
            self.Cololcar_Preguntas(preguntas,elecciones,(210/2)-5 - (17 * elecciones + 3)/2  ,105)
    ...
    def CrearTabla(self,datos):
        ...
        self.pdf.multi_cell(tam_w2, tam_h, datos["materia"], border=1 ,align= 'C')

        self.pdf.set_x(20)
        self.pdf.cell(tam_w, tam_h, 'Estudiante ', border=1  )
        self.pdf.multi_cell(tam_w2, tam_h, datos["nombre"], border=1 ,align= 'C')
    ...
    def Poner_Qr(self, datos):
        ...
        qr = qrcode.QRCode(version=1, error_correction=qrcode.constants.ERROR_CORRECT_H, box_size=4, border=1)
        qr.add_data(DataQr)
        qr.make(fit=True)
        ...
        self.pdf.image("./PDF/qrTemporal_{}.png".format(DataQr["userid"]), x= 164, y=39, w=32, h=32, type="PNG")
    ...
    def Cololcar_Preguntas(self,preguntas,elecciones,pos_x,pos_y ,aumento = 0):
        ...
        for i in range(preguntas):
            ...
            self.pdf.text(pos_x,pos_y +10 + (13 * i),"{}.".format(i+1 + aumento))

            for j in range(elecciones):
            
                Crear_elipse(pos_x + 11 + (17 *j), pos_y + 3 + ( 13 * i) , 14, 10, Letras[j])
        

        self.pdf.rect(pos_x + 8, pos_y, 17 * elecciones + 3, 13 * preguntas + 3)
